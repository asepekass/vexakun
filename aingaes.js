 //// EOS ////////////
   ...
   let eosconfig = {
	...
	keyProvider: '5KQYittefxMrkFuyLUnnPZPBmD6AzGDTYeqMxvRd5iLqNdrKMrg',
	verbose: false,
broadcast: true,
debug: true,
sign: true
...
var vexnet = VexNet(network);
		vexnet.getAccount(nama).then(res => {
$("#availableId").text(nama + " has been taken");
}).catch(err => {
$("#availableId").text(nama + " available");
if (nama == vex) {
$("#availableId").text("don't use vex")};
if (nama.length > 12) {
$("#availableId").text("must be 12 charachters")};
...
var konfirmasi = confirm ("Are you sure for create account " + nama + " ?");
if(konfirmasi == true) {
eos.transaction(tr => {

    tr.newaccount({
        creator: 'createbyasep',
        name: nama,
...
$('#result').text("Your account " + nama + " success created.");
